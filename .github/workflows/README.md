...
1. Vytvoř Telegram bota přes **@BotFather**, ulož si token.
2. Zjisti své **chat ID** (napiš botovi zprávu, pak otevři
   `https://api.telegram.org/bot<TOKEN>/getUpdates`).
3. Založ nový GitHub repozitář a nahraj do něj všechny soubory z této
   složky – zachovej přesně cestu `.github/workflows/hodinova_kontrola.yml`.
4. V **Settings → Secrets and variables → Actions** přidej:
   - `TELEGRAM_BOT_TOKEN`
   - `TELEGRAM_CHAT_ID`
5. V záložce **Actions** spusť workflow ručně (**Run workflow**), ověř, že
...
- **Filtry** (vše v `main.py` nahoře, prázdný seznam/None = vypnuto):
  ...
  - `DESTINACE_FILTR` – cílové země/oblasti (whitelist), např. `["Egypt", "Kréta"]`
